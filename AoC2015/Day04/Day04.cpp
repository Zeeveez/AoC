...
namespace AoC2015 {
    namespace Day04 {
        ...
        std::tuple<uint32_t, uint32_t, uint32_t, uint32_t> MD5(std::string input) {
        ...
        uint64_t Test(std::string input, std::function<bool(std::tuple<uint32_t, uint32_t, uint32_t, uint32_t>)> validityCheck) {
            int i = 0;
            while (true) {
                auto testInput = input + std::to_string(++i);
                if (validityCheck(MD5(testInput))) { return i; }
            }
        }
        ...
        std::pair<uint64_t, std::chrono::duration<double, std::milli>> A(const std::string& input) {
            ...
            auto r = Test(input, [](std::tuple<uint32_t, uint32_t, uint32_t, uint32_t> t) { return (std::get<0>(t) & 0x00000FFF) == std::get<0>(t); });
        ...
        std::pair<uint64_t, std::chrono::duration<double, std::milli>> B(const std::string& input) {
            ...
            auto r = Test(input, [](std::tuple<uint32_t, uint32_t, uint32_t, uint32_t> t) { return (std::get<0>(t) & 0x000000FF) == std::get<0>(t); });
